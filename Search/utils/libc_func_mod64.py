...
def __strdup(ctx, tainted_flag, ast):
    global mallocCurrentAllocation
    global mallocMaxAllocation
    global mallocBase
    global mallocChunkSize
    ...
    # Get arguments
    str_location = ctx.getConcreteRegisterValue(ctx.registers.rdi)
    # 
    target_string = getMemoryString(ctx,str_location)
    size = len(target_string) # 这里写传入的字符串的内存大小
    if size > mallocChunkSize:
        print('malloc failed: size too big')
        sys.exit(-1)
    ...
    area_dest = mallocBase + (mallocCurrentAllocation * mallocChunkSize)
    for idx in range(size):
        # 向目标中复制内存
        ctx.setConcreteMemoryAreaValue(area_dest+idx,bytes(target_string[idx].encode()))
    mallocCurrentAllocation += 1
    # 这里检查如果rdi是污染的内存，那么就可以直接完成对任意内存的复制改写
    if ctx.isRegisterTainted(ctx.registers.rdi):
        ctx.taintRegister(ctx.registers.rax)
    return  area_dest, ctx
...
customRelocation64 = [
    ...
    ['sudo_debug_exit_str_masked_v1', __ignore_func, None],
    ['_errno_location', __ignore_func,           None],
    ['errno_location', __ignore_func,           None],
    ['sigaction',      __ignore_func,           None],
    ['sudo_term_restore_v1',  __ignore_func,    None],
    ['strdup',         __strdup,                None],
    ['sudo_memset_s',  __memset,                None],
    ['sudo_debug_set_active_instance_v1', __ignore_func, None],
    ...
    ['abort', __abort, None],
    ['gst_debug_log', __ignore_func, None],
